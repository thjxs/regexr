...
export default class Reference {
	...
	search(searchStr) {
		...
		return srch(this.content.kids, []).sort(
			(a, b) => b.__searchPoints - a.__searchPoints
		);
	}
	...
	getError(error, token) {
		...
		let str =
			this._content.errors[errId] || "no docs for error='" + errId + "'";
		if (token) {
	...
	tipForToken(token) {
		...
		if (token.error && !token.error.warning) {
			...
			if (token.error) {
				tip +=
					"<span class='warningtext'><span class='error warning'>WARNING: </span>" +
					this.getError(token.error, token) +
					"</span>";
			}
		}
		...
		return tip
			? label + tip
			: "no docs for id='" + this.idForToken(token) + "'";
	}

	getContent(id) {
		let node = this.getNode(id);
		return this.fillTags(
			this.getVal(node, "desc") + this.getVal(node, "ext"),
			node,
			this
		);
	}
	...
	tipForMatch(match, text) {
		...
		let str =
			"<b>match: </b>" +
			Utils.shorten(text.substr(match.i, match.l), 150, true, "i") +
			"<br/><b>range: </b><code>" +
			match.i +
			"-" +
			(match.i + match.l - 1) +
		...
		for (let i = 0; i < l; i++) {
			if (i > 3 && l > 5) {
				more = false;
				str +=
					"<br><span class='more'>see Details for " + (l - i) + " more</span>";
				break;
			}
			let group = groups[i],
				s;
			...
			str += i > 0 ? "<br>" : "<hr>";
			str +=
				"<b>group #" + (i + 1) + ": </b>" + Utils.shorten(s, 50, true, "i");
		}
		if (more) {
	...
	_updateHide(o, list) {
		...
		if (kids) {
			...
			hide =
				o.show === false ||
				(o.show !== true && o.id && !this._flavor.isTokenSupported(o.id));
		}
		if (list && hide) {
			list.push(o.id);
		}
