...
export default class Explain {
	constructor(el) {
		...
		app.expression.highlighter.addEventListener(
			"hover",
			this._bound_handleEvent
		);
	}
	...
	cleanup() {
		...
		app.expression.highlighter.removeEventListener(
			"hover",
			this._bound_handleEvent
		);
	}
	...
	// private methods:
	_update() {
		let el = $.empty(this.el),
			token = app.expression.token,
			expr = app.expression.value;
		this._divs = [];
		if (!token || token.next.type === "close") {
			el.innerHTML =
				"<span class='desc'>Enter an Expression above and it will be explained here.</span>";
			return;
		}
		el.innerHTML =
			"<span class='desc'>Roll-over elements below to highlight in the Expression above. Click to open in Reference.</span>";
		while ((token = token.next) && token.type !== "close") {
			...
			if (token.set) {
				...
				content =
					...
					(set0.clss || set0.type) +
					"'>" +
					expr.substring(set0.i, set0.i + set0.l) +
					"</span>";
				...
				content +=
					...
					(set2.clss || set2.type) +
					"'>" +
					expr.substring(set2.i, set2.i + set2.l) +
					"</span>";
			}
			...
			if (token.close) {
				...
				if (token.depth > 3) {
					div.innerHTML =
						"So... you wanted to see what would happen if you just kept nesting groups, eh? Well, this is it." +
	...
	_handleMouseEvent(evt) {
		...
		if (type == "click") {
			...
			app.expression.highlighter.hoverToken =
				type === "mouseout" ? null : token;
		}
		evt.stopPropagation();
	}
