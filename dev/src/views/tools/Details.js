...
export default class Details {
	...
	_update() {
		...
		$.create(
			"div",
			"desc",
			"Click a <span class='match'>match</span> above to display match &amp; group details. Mouse over a <code>Group</code> row to highlight it in the Expression.",
			this.el
		);
			...
			for (let i = 0; i <= l; i++) {
				...
				if (ext) {
					for (let j = inGroups.length - 1; j >= 0; j--) {
						...
						inGroups.pop();
						extStr +=
							Utils.htmlSafe(textVal.substring(lastIndex, ge)) + "</span>";
						lastIndex = ge;
					}
				}
				if (!group) {
				...
				if (group.l) {
					extStr +=
						Utils.htmlSafe(textVal.substring(lastIndex, index)) +
						"<span class='group-" +
						(num % 6) +
						" num-" +
						num +
				...
				let val =
					"<span" +
					(ext ? " class='group-" + (num % 6) + "'" : "") +
					">" +
					this._getMatchVal(group, textVal) +
					"</span>";
	...
	_handleMouseEvent(evt) {
		...
		if (type === "mouseover") {
			$.addClass($.query("span.num-" + token.num, this.el), "hover");
		} else {
			$.removeClass($.query("span.hover", this.el), "hover");
		}
		evt.stopPropagation();
	}
}
