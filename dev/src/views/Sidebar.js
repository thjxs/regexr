...
export default class Sidebar {
	...
	show(item) {
		...
		if (item.id) {
			if (
				item.id === "home" ||
				(item.parent && item.parent.id === "home") ||
				this._isInReference(item)
			) {
		...
		$.query("svg.back.icon use", this.titleEl).setAttribute(
			"xlink:href",
			"#" + (item.parent ? "arrowleft" : "menu")
		);
		if (item.el) {
		...
		if (item.search) {
			$.removeClass(this.fullEl, "no-search");
		}
	...
	menuListTemplate(o) {
		return (
			(o.parent && o.parent.id === "home"
				? '<svg class="icon"><use xlink:href="#' + o.id + '"></use></svg>'
				: "") +
			'<span class="label">' +
			(o.label || o.id) +
			"</span>" +
			(o.token
				? '<span class="token">' + o.token.replace("<", "&lt;") + "</span>"
				: "") +
			(o.kids || o.el
				? '<svg class="small icon"><use xlink:href="#arrowright"></use></svg>'
				: "")
		);
	}
	...
	communityListTemplate(o) {
		return (
			'<span class="label">' +
			Utils.htmlSafe(o.name) +
			"</span>" +
			'<span class="rating">' +
			(o.favorite
				? '<svg class="small icon favorites"><use xlink:href="#favorites"></use></svg>'
				: "") +
			'<svg class="small icon thumb"><use xlink:href="#thumb"></use></svg>' +
			o.rating.toFixed(1) +
			"</span>"
		);
	}
	...
	_initUI(el) {
		...
		$.query("> .close.icon", this.titleEl).addEventListener("click", () =>
			this.minimize(true)
		);
		$.query("> .backrow", this.titleEl).addEventListener("click", () =>
			this.back()
		);
		...
		this.searchFld.addEventListener(
			"keyup",
			(evt) => evt.keyCode === 13 && this._onSearchSubmit()
		);
		...
		this.menuList.on("dblclick", () =>
			this._onDblClick(this.menuList.selectedItem)
		);
	...
	_showContent(o) {
		if (
			(this.curItem.id === "community" || this.curItem.id === "favorites") &&
			o !== this.curItem
		) {
			...
			this.contentEl.innerHTML = this._isInReference(o)
				? ref.getContent(o.id)
				: ref.fillTags((o.desc || "") + (o.ext || ""), o, ref);
			if (o.example) {
	...
	_onDblClick(o) {
		if (o.token) {
			...
			this.curItem.id === "community" ||
			this.curItem.id === "favorites"
		) {
			app.load(o);
		}
	}
	...
	_showServerResults(data) {
		...
		if (data.results.length === 0) {
			this._showListMsg(
				this.curItem.id === "community"
					? "No matches."
					: "No patterns created or favorited."
			);
		}
	}
