...
export default class Expression extends EventDispatcher {
	...
	_initUI(el) {
		...
		this.errorEl.addEventListener("mouseenter", (evt) =>
			this._onMouseError(evt)
		);
		this.errorEl.addEventListener("mouseleave", (evt) =>
			this._onMouseError(evt)
		);
	...
	_initTooltips(el) {
		...
		$.query(".icon.help", this.flagsEl).addEventListener("click", () =>
			app.sidebar.goto("flags")
		);
	}
	...
	_onFlavorChange() {
		...
		let supported = Expression.FLAGS.split("").filter(
			(n) => !!profile.flags[n]
		);
		...
		this.flags = this.flags
			.split("")
			.filter((n) => !!profile.flags[n])
			.join("");
	}

	_onEditorMouseDown(cm, evt) {
		// offset by half a character to make accidental clicks less likely:
		let index = CMUtils.getCharIndexAt(
			cm,
			evt.clientX - cm.defaultCharWidth() * 0.6,
			evt.clientY
		);
	...
	_onMouseError(evt) {
		...
		let str = err
			? app.reference.getError(err, errs[0])
			: "Problems in the Expression are underlined in <span class='exp-error'>red</span>. Roll over them for details.";
		let label = err && err.warning ? "WARNING" : "PARSE ERROR";
		tt.showOn(
			"error",
			"<span class='error'>" + label + ":</span> " + str,
			this.errorEl
		);
	}
}
