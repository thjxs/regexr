...
export default class Share extends EventDispatcher {
	...
	_initUI() {
		...
		$.query(".savekey", this.hSaveBtn).innerText =
			"(" + Utils.getCtrlKey() + "-s)";
		...
		$.query(".row.signin a", this.mainEl).addEventListener("click", () =>
			this._doSignin()
		);
		...
		$.query(".button.cancel", comEl).addEventListener("click", () =>
			app.sidebar.goto("share")
		);
		$.query(".button.share", comEl).addEventListener("click", () =>
			this._doComSave()
		);
	...
	_updateUI() {
		...
		if (!isOwned) {
			text =
				"This pattern was created by '" + (o.author || "[anonymous]") + "'.";
		} else if (!isChanged) {
		...
		if (!isOwned && !isChanged) {
			...
			text +=
				" Fork will create a new copy" +
				(isChanged ? " with your changes." : ".");
		}
	...
	_handleSave(data) {
		...
		if (isFork || isNew) {
			...
			this._linkRow.showMessage(
				"<b>Saved.</b> New share link created. Click to copy to clipboard."
			);
		}
	}
		...
		setTimeout(
			() =>
				app.tooltip.toggle.showOn(
					"delete",
					"Pattern was permanently deleted.",
					this._deleteRow,
					true,
					0
				),
	...
	_handleAccountChange() {
		let acc = app.account,
			rowEl = $.query(".signin.row", this.mainEl);
	...
	_getErrMsg(err) {
		return (
			...
			app.reference.getError("servercomm")
		);
	}
}
