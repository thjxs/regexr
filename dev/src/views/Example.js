...
export default class Example {
	...
	set example(ex) {
		...
		if (ex) {
			...
			str +=
				"<code class='expression'><svg class='icon load'><use xlink:href='#load'><title>Load expression</title></use></svg>" +
				Utils.htmlSafe(exp) +
				"</code>";
			if (txt && regex) {
				...
				str +=
					"<hr><code class='text'><svg class='icon load'><use xlink:href='#load'><title>Load text</title></use></svg>" +
					s +
					(over ? "<i>\u2026</i>" : "") +
					"</code>";
			}
		...
		if (exp) {
			$.query("code.expression > .load", this.el).addEventListener(
				"click",
				() => {
		...
		if (txt) {
			$.query("code.text > .load", this.el).addEventListener(
				"click",
				() => (app.text.value = txt)
			);
		}
